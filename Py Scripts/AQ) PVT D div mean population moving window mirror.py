...
def main():
    ...
    plot_name = "PVT_CUM_NUM_VX_UVX PVT_CUM_NUM_VX_UVX_D WND_7"
    # Read the CSV files
    population_df = pd.read_csv(r"C:\github\CzechFOI-BYAGE\TERRA\PVT_CUM_NUM_VX.csv", sep=',')
    deaths_df = pd.read_csv(r"C:\github\CzechFOI-BYAGE\TERRA\PVT_NUM_VX_D.csv", sep=',')
    population_df_mir = pd.read_csv(r"C:\github\CzechFOI-BYAGE\TERRA\PVT_CUM_NUM_UVX.csv", sep=',')
    deaths_df_mir = pd.read_csv(r"C:\github\CzechFOI-BYAGE\TERRA\PVT_NUM_UVX_D.csv", sep=',')
    ...
    df_mir = population_df_mir.copy()
    
    for age_group in population_df.columns[1:]:
        df[age_group] = population_df[age_group] - deaths_df[age_group]

    for age_group in population_df_mir.columns[1:]:
        df_mir[age_group] = population_df_mir[age_group] - deaths_df_mir[age_group]
    ...
    for age_group in df.columns[1:]:
        ...
        for day in range(range_value):  # Move by one day each time
            #Collect the pivot DataFrame data#
            
            for future_day in range(day, range_value):  
                if future_day + window_size < range_value:        
                    mean_population = df[age_group].iloc[future_day:future_day + window_size].mean()
                    deaths = deaths_df[age_group].iloc[future_day:future_day + window_size].sum()
                    mean_population_mir = df_mir[age_group].iloc[future_day:future_day + window_size].mean()
                    deaths_mir = deaths_df_mir[age_group].iloc[future_day:future_day + window_size].sum()

                    if mean_population > 0:
                        value = deaths / mean_population
                    else:
                        value = 0
                    if mean_population_mir > 0:
                        value_mir = deaths_mir / mean_population_mir
                    else:
                        value_mir = 0

                    #print(f"value:{value} fday:{future_day} day: {day} ag: {age_group}")
                    pivot_df.at[day, future_day] = value
                    pivot_df.at[range_value - day, future_day] = value_mir

        print(f"append:pivot_df :{day}")
        ...
        fig.update_layout(
            title=f'Heatmap death div mean population sliding window mirrord: {window_size} AG: {age_group}',
        ...
        print(f"Plot has been saved to HTML file {full_plotfile_name} {age_group}.html")

...
def main():
    ...
    plot_name = "PVT_CUM_NUM_D PVT_CUM_NUM"
    ...
    for age_band in age_bands:
        ...
        # Calculate and add traces for the first derivatives on a third y-axis
        for df, file in zip(dataframes_primary, csv_files_primary):
            derivative = np.gradient(df[age_band], df.iloc[:, 0])
            fig.add_trace(go.Scatter(x=df.iloc[:, 0], y=derivative, mode='lines', name=f'{file} (Derivative)', line=dict(dash='dash'), yaxis='y3'))
        ...
        print("Plots have been saved to HTML files.")

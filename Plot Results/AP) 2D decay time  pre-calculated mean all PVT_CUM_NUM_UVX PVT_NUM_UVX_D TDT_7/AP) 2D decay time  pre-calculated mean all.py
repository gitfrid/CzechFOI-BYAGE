...
def main():
    ...
    for age_group in df.columns[1:]:
        ...
        #print(decay_times_df)

        # Calculate rolling average for 'Decay Time' with a window of 7 days
        decay_times_df[f'{age_group} Rolling Avg'] = decay_times_df[age_group].rolling(window=7, min_periods=1).mean()
    ...
    fig = px.line(decay_times_long, x='DAYS_20200101', y='Decay Time', color='Age Group', 
                title='Decay Time by Age Group over Days - biger values are better')
    ...
    # Now add the rolling averages
    # Convert rolling averages to long format and add to the plot
    rolling_avg_long = decay_times_df.melt(id_vars=['DAYS_20200101'], value_vars=[f'{age_group} Rolling Avg' for age_group in df.columns[1:]],

...
def main():
    ...
    # Load the CSV file
    file_path = r'C:\CzechFOI-BYAGE\TERRA\PVT_NUM_UVX_D_NORM.csv'
    data = pd.read_csv(file_path)

    # Apply a logarithmic transformation to the data to enhance small values
    data_transformed = data.iloc[:, 1:].map(lambda x: np.log1p(x))

    # Define custom range for the color axis
    zmin = data_transformed.min().min()
    zmax = data_transformed.max().max()
    ...
    fig.update_layout(
        title=f"2D Heatmap of Log-Transformed Values by Days and Age Groups {plot_name}",

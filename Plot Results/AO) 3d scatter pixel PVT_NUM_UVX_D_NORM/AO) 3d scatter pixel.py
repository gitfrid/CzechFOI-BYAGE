...
def main():
    ...
    # Load the CSV file
    df = pd.read_csv(r'C:\github\CzechFOI-BYAGE\TERRA\PVT_NUM_UVX_D_NORM.csv')
    ...
    z = df.iloc[:, 1:].values.flatten()  # Flatten the 2D array to 1D

    # Initialize the directory and script copy
    full_plotfile_name = init_function(plot_name)

    # Create 3D scatter plot with pixel-like points and color range from 0 to max z value
    ...
    # Update layout for better visualization and set z-axis limit
    fig.update_layout(
        title='3D Scatter Plot with Pixel-like Points',
        scene=dict(
            ...
            range=[0, 1000])  # Set z-axis range from 0 to 1000
        )
    )
